...
if __name__ == '__main__':
    ...
    model.add(Dense(units=1500,
                    ...
                    activation='relu'))
    model.add(Dropout(0.3))
    model.add(Dense(units=2,    # ensure the number of the label is two
                    kernel_initializer='normal',
                    activation='softmax'))

    print(model.summary())  # show the layers of the model
    ...
    #                     epochs=epochs)
    #
    # model.save(filepath='car1.keras', save_format='keras')

    model.load_weights(filepath='models/car1.keras')  # skip training and use the saved model
